...
def get_severity(rule_name):
    if (
        "rules" in config
        and rule_name in config["rules"]
        and "severity" in config["rules"][rule_name]
    ):
...
if len(sys.argv) != 3:
    print(
        "Usage: build_checks.py <path to ShellCheck Wiki pages> <YAML file with rule severities>",
        file=sys.stderr,
    )
    sys.exit(1)
sc_path = sys.argv[1]
...
try:
    config = yaml.load(
        open(os.path.dirname(os.path.abspath(__file__)) + "/build_checks.yml").read(),
...
for rule in config["rules"]:
    if get_config(rule, "ignored"):
        continue
    if (
        not os.path.isfile(rule + ".html")
        or not os.path.isfile(rule + ".json")
        ...
        json_data["remediation"] = {
            "func": "Constant/Issue",
            "constantCost": get_config(rule, "constantCost"),
        }
